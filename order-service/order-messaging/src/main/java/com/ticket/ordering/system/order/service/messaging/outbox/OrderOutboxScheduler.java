...
public class OrderOutboxScheduler {
    ...
    public OrderOutboxScheduler(OrderOutboxRepository orderOutboxRepository,
    ...
    public void publishPendingMessages() {
        ...
        orderOutboxRepository.findPendingMessages(maxAttempts, processingTimeout)
                .forEach(this::publishMessage);
    }

    private void publishMessage(OrderOutboxMessage outboxMessage) {
        if (!orderOutboxRepository.markAsProcessing(outboxMessage.getId())) {
            return;
        }
            ...
            kafkaProducer.send(outboxMessage.getTopicName(), outboxMessage.getMessageKey(), payload,
                    new KafkaCallback<>() {
                        ...
                        public void onSuccess(String topicName, String key, Object message) {
                            orderOutboxRepository.markAsCompleted(outboxMessage.getId());
                            log.info("Order outbox message {} sent to topic {} with key {}",
                                    outboxMessage.getId(), topicName, key);
                        }

                        @Override
                        public void onFailure(Throwable throwable) {
                            orderOutboxRepository.markAsFailed(outboxMessage.getId(), errorMessage(throwable));
                            log.error("Failed to send order outbox message {} to topic {}",
                                    outboxMessage.getId(), outboxMessage.getTopicName(), throwable);
                        }
                    });
        } catch (Exception e) {
            orderOutboxRepository.markAsFailed(outboxMessage.getId(), errorMessage(e));
            log.error("Failed to publish order outbox message {}", outboxMessage.getId(), e);
        }
    }
